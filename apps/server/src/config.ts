...
export function loadConfig(): ServerConfig {
  ...
  return {
    ...
    cnameTarget: process.env.CNAME_TARGET ?? (deployTarget === "self-hosted" ? (serverIp ?? platformZone) : `customers.${platformZone}`),
    fallbackOrigin: process.env.FALLBACK_ORIGIN ?? (deployTarget === "self-hosted" ? (serverIp ?? platformZone) : `origin.${platformZone}`),
    platformZone,
    deployTarget,
    serverIp,
